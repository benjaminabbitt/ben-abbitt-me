...
**Why `.well-known/`?** Roy Fielding's [objection to `contribute.json`](https://mailarchive.ietf.org/arch/msg/wellknown-uri-review/tf4EY_QCSlunyUZ1pbkGhKT6jQI/) — that "reserving a universal location for this purpose doesn't make any sense" when clients could fetch a linked document from a homepage — applies to any well-known proposal and must be answered. The answer for `well-known-spec`:
...
**Cache and `Cache-Control`.** The canonical versioned URL is immutable per §4.1 and SHOULD be served with `Cache-Control: public, max-age=31536000, immutable` per §4.2. The "directory" paths (the convention root and the per-spec directory) have no caching guidance because they have no normative semantics.
