...
This post uses "bounded context" when discussing DDD theory.
:::
...
|---------|---------------|-------------------|
| Sagas emitting commands | Avoid—events only | Supported but discouraged; prefer facts |
| Cascading synchronous calls | Never | Supported for legacy boundaries; refactor when possible |
| Undersized aggregates requiring coordination | Anti-pattern | Supported with compensation tooling; indicates boundary debt |
