...
description: "Most DDD bounded contexts are drawn too small. A context is correctly sized when every decision that changes its invariants can be made entirely within it."
publishDate: "2026-03-11"
tags: ["software-architecture"]
marginalia: "Most DDD teams draw bounded contexts too small. The right size: every decision that changes invariants can be made entirely within the context."
---
...
This post uses "bounded context" when discussing DDD theory. See the [glossary](/glossary) for the full mapping.
:::
...
The principles above represent mainstream DDD thinking. But having built [Angzarr](/getting-started)—an event-sourcing framework for distributed systems—I've encountered cases where rigid adherence to these rules creates its own problems.
...
|---------|---------------|-------------------|
| Sagas emitting commands | Avoid—events only | Supported but discouraged; prefer [facts](/features/facts) |
| Cascading synchronous calls | Never | Supported for legacy boundaries; refactor when possible |
| Undersized aggregates requiring coordination | Anti-pattern | Supported with [compensation](/features/compensation) tooling; indicates boundary debt |
