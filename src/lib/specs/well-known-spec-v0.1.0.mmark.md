...
well-known-spec is the path-structure convention that fills this gap, designed to compose with -- not replace -- existing discovery layers. An api-catalog linkset entry can point at /.well-known/spec/{name}/v{x.y.z}.{ext} URLs, gaining versioning, immutability, format-selection, and origin-binding semantics for free; the discovery layer handles "what specs and what current versions does this origin advertise" while the publishing layer handles "where does each version-pinned document live, and how is it referenced." The two layers compose cleanly because they answer different questions. The need for the publishing layer is not hypothetical; it is documented in adjacent standards as a gap they declined to fill.
...
Different formats carry different content -- by design. The byte content at each URL is expected to differ, sometimes substantially. Narrative formats (.md, .txt, .html, .pdf) typically include prose, rationale, examples, references, and discussion that the format permits. Machine-readable formats (.proto, .json, .yaml for OpenAPI/JSON Schema/etc., .cddl, .asn1) are tightly bound to the data shape they describe; rationale typically lives in comments or is absent entirely. A consumer fetching .proto is asking for the IDL; a consumer fetching .md is asking for the IDL and the prose explaining it. Both are valid views of the same spec.

Cross-format consistency (normative): documents at the same {name} and same {x.y.z} MUST describe the same thing. They are not independent specs that happen to share a base name; they are different presentations -- at different levels of formality and detail -- of one spec. The normative content (rules, data shape, structural requirements, identifiers, version constraints) MUST agree across formats. Differences MUST be one of:

* Additions. Prose, rationale, examples, references, diagrams, or commentary present in a narrative format and absent from a machine-readable format. These are permitted and expected; the narrative is the rationale layer over the same underlying spec.
* Presentation. Styling, layout, anchors, pagination, table-of-contents rendering. The same content arranged for the medium.
* Format-imposed structure. A .proto file expresses the same shape as a .md description of that shape, in the syntax .proto requires. The shape is the same; the surface is different.

Differences MUST NOT be contradictions. If the .proto declares a required field and the .md says it is optional, that is a contract violation. If the .json schema permits a value the .md forbids, that is a contract violation. The machine-readable form and the narrative form must describe the same normative reality.
